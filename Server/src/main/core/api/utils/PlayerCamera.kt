...
class PlayerCamera(val player: Player) {
    ...
    fun setPosition(x: Int, y: Int, height: Int) {
        player ?: return
        ctx = Context.Camera(player, Context.Camera.CameraType.SET, x, y, height, 0, 0)
    ...
    fun rotateTo(x: Int, y: Int, height: Int, speed: Int) {
        player ?: return
        ctx = Context.Camera(player, Context.Camera.CameraType.ROTATION, x, y, height, speed, 1)
        PacketRepository.send(CameraViewPacket::class.java, ctx!!)
    }
    ...
    fun rotateBy(diffX: Int, diffY: Int, diffHeight: Int, speed: Int) {
        player ?: return
        ctx ?: return
        ctx = Context.Camera(player, Context.Camera.CameraType.ROTATION, ctx!!.x + diffX, ctx!!.y + diffY, ctx!!.height + diffHeight, speed, 1)
        PacketRepository.send(CameraViewPacket::class.java, ctx!!)
    }
    ...
    fun panTo(x: Int, y: Int, height: Int, speed: Int) {
        player ?: return
        ctx = Context.Camera(player, Context.Camera.CameraType.POSITION, x, y, height, speed, 1)
        PacketRepository.send(CameraViewPacket::class.java, ctx!!)
    }
    ...
    fun shake(cameraType: Int, jitter: Int, amplitude: Int, frequency: Int, speed: Int) {
        player ?: return
        ctx = Context.Camera(player, Context.Camera.CameraType.SHAKE, cameraType, jitter, amplitude, frequency, speed)
        PacketRepository.send(CameraViewPacket::class.java, ctx!!)
    }
    ...
    fun shake(cameraType: CameraShakeType, jitter: Int, amplitude: Int, frequency: Int, speed: Int) {
        val ctx = Context.Camera(player, Context.Camera.CameraType.SHAKE, cameraType.ordinal, jitter, amplitude, frequency, speed)
        PacketRepository.send(CameraViewPacket::class.java, ctx)
    }
    ...
    fun reset() {
        player ?: return
        ctx = Context.Camera(player, Context.Camera.CameraType.RESET, -1, -1, -1, -1, -1)
        PacketRepository.send(CameraViewPacket::class.java, ctx!!)
    }
}
