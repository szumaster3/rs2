...
public final class CommunicationInfo {
    ...
    public static void add(Player player, String contact) {
        ...
        if (target != null) {
            if (showActive(player, target)) {
                PacketRepository.send(ContactPackets.class, new Context.Contact(player, contact, GameWorld.getSettings().getWorldId()));
            }
            if (player.getSettings().getPrivateChatSetting() == 1 && showActive(target, player)) {
                PacketRepository.send(ContactPackets.class, new Context.Contact(target, player.getName(), GameWorld.getSettings().getWorldId()));
            }
        }
    }
    ...
    public static void remove(Player player, String contact, boolean block) {
        ...
        if (block) {
            ...
            if (target != null && hasContact(target, player.getName())) {
                ...
                PacketRepository.send(ContactPackets.class, new Context.Contact(target, player.getName(), worldId));
            }
        } else {
            ...
            if (player.getSettings().getPrivateChatSetting() == 1) {
                Player target = Repository.getPlayerByName(contact);
                if (target != null) {
                    PacketRepository.send(ContactPackets.class, new Context.Contact(target, player.getName(), 0));
                }
            }
        }
    }
    ...
    public static void updateClanRank(Player player, String contact, ClanRank clanRank) {
        ...
        PacketRepository.send(ContactPackets.class, new Context.Contact(player, contact, worldId));
    }
