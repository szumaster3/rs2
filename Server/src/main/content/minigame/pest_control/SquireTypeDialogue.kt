...
class SquireTypeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npc("The Void Knights, they are great warriors of balance", "who do Guthix's work here in " + settings!!.name + ".").also { stage = END_DIALOGUE }
            5 -> npc("This is where we launch our lander to combat the", "invasion of the nearby islands. You'll see three lander - ", "one for novice, intermediate and veteran adventurers.", "Each has a different difficulty, but they therefore offer").also { stage++ }
            6 -> npc("varying rewards.").also { stage++ }
            7 -> player("And this lander is...?").also { stage++ }
            8 -> npc("The " + (when (npc.id) {
                3802 -> {
                    "novice"
                }
                6140 -> {
            ...
            10 -> npc("Simple. We send each lander out every five minutes,", "however we need at least five volunteers or it's a suicide", "mission. The lander can only hold a maximum of", "twenty five people though, so we do send them out").also { stage++ }

            11 -> npc(
                "early if they get full. If you'd be willing to help us then",
                "just wait in the lander and we'll launch it as soon as it's",
                "ready. However you will need a combat level of " + (when (npc.id) {
                    3802 -> {
                        "40"
                    }
                    6140 -> {
