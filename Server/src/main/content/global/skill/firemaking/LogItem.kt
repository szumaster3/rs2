 ...
 * Represents logs.
 */
enum class LogItem(val logId: Int, val defaultLevel: Int, val barbarianLevel: Int, val life: Int, val fireId: Int, val xp: Double) {
    ...
    companion object {
        private var logItemMap: HashMap<Int, LogItem> = HashMap()

        init {
            for (log in values()) {
                logItemMap[log.logId] = log
            }
        }

        @JvmStatic
        fun forId(id: Int): LogItem? = logItemMap[id]
    }
}
