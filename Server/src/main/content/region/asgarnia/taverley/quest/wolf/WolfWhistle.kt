...
/**
 * The type Wolf whistle.
 */
@Initializable
class WolfWhistle : Quest(Quests.WOLF_WHISTLE, 146, 145, 1) {
        ...
        if (stage == 0) {
            ...
            if (stage >= 60) {
                ...
                if (player.getSkills().getLevel(Skills.SUMMONING) >= player.getSkills()
                        .getStaticLevel(Skills.SUMMONING) || stage >= 100
                ) {
    ...
    override fun finish(player: Player) {
        super.finish(player)
        drawReward(player, "1 Quest Points", 10)
        drawReward(player, "276 Summoning XP", 11)
    ...
    override fun getConfig(player: Player, stage: Int): IntArray {
        val `val` = getVarp(player, Vars.VARP_QUEST_WOLF_WHISTLE_PROGRESS_1178)
        val open = `val` >= 4096
        val closed = `val` == 2048
        if (stage == 100) {
            if (`val` == 5 || `val` == 0) {
                return intArrayOf(1178, 32989)
            } else if (`val` == 4101) {
                return intArrayOf(1178, 28893)
            }
            return intArrayOf(1178, `val`)
        }
        return intArrayOf(
            1178,
            if (stage > 0) 5 + (if (open) 4096 else 0) else if (stage >= 100) if (!closed) 28893 else 32989 else 0
        )
    }
}
