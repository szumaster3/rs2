...
class LubufuDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> when (buttonId) {
                1 -> player(FaceAnim.NEUTRAL, "I wasn't going anywhere...").also { stage++ }
                2 -> player(FaceAnim.HALF_ASKING, "What's a whippersnapper?").also { stage = 3 }
                3 -> player(FaceAnim.HALF_ASKING, "Who are you?").also { stage = 4 }
            }
            ...
            4 -> npc(FaceAnim.NEUTRAL, "I am Lubufu - the only fisherman who knows the secret", "of the Karambwan!").also { stage++ }
            5 -> player(FaceAnim.HALF_ASKING, "What's a Karambwan?").also { stage++ }
