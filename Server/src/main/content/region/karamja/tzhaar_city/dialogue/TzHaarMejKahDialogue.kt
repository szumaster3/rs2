package content.region.karamja.tzhaar_city.dialogue
...
class TzHaarMejKahDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            10 -> npc(FaceAnim.CHILD_GUILTY, "This is the fight pit, TzHaar-Xil made it for their sport", "but many JalYt come here to fight too.", "If you are wanting to fight then enter the cage, you", "will be summoned when the next round is ready to begin.").also { stage++ }
            ...
            20 -> npc(FaceAnim.CHILD_GUILTY, "Gold is like you JalYt, soft and easily broken, we use", "hard rock forged in fire like TzHaar!").also { stage = END_DIALOGUE }
            21 -> npc(FaceAnim.CHILD_GUILTY, "Ah that would be " + npc.getAttribute("fp_champion", "JalYt-Ket-Emperor")).also { stage = END_DIALOGUE }
        }
        return true
    }
