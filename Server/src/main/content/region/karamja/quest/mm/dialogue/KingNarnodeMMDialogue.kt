...
class KingNarnodeMMDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (getQuestStage(player!!, Quests.MONKEY_MADNESS)) {
                ...
                21 -> end().also {
                    ...
                    for (i in 0..17) {
                        when (i) {
                            ...
                            9 -> sendString(player!!, "In which our ${if (player!!.isMale) "hero finds himself" else "heroine finds herself"} drawn back into Glough's", interfaceId, i)
            ...
            10 -> npcl("Welcome back, adventurer.").also { stage = END_DIALOGUE }
            11 -> when (stage) {
