...
class OliviaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        options("Would you like to trade in seeds?", "Where do I get higher-level seeds?", "About the recent bank robbery...")
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> when (buttonId) {
                ...
                3 -> player(FaceAnim.ASKING, "About the recent bank robbery...", "The robber attacked you, didn't he?").also { stage = 3 }
            }
            1 -> end().also { openNpcShop(player, npc.id) }
            2 -> npc(FaceAnim.FRIENDLY, "The Master Farmers usually carry a few rare seeds", "around with them, although I don't know if they'd want", "to part with them for any price to be honest.").also { stage = END_DIALOGUE }
            ...
            10 -> npc(FaceAnim.HALF_ASKING, "You're welcome. Would you like to see my fine", "selection of seeds?").also { stage++ }
