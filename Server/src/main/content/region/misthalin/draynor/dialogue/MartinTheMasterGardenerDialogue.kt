...
class MartinTheMasterGardenerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            20 -> end()
            10 -> {
                npc(FaceAnim.HALF_GUILTY, "This is a Skillcape of Farming, isn't it incredbile? It's a", "symbol of my ability as the finest farmer in the land!")
                stage = 11
            }

            11 -> if (getStatLevel(player, Skills.FARMING) == 99) {
